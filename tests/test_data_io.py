...
def test_import_data_restores_owner_and_pets():
    ...
    restored_owner, restored_pets = import_data(json_str)
...
def test_import_data_restores_tasks():
    owner, pets = _sample_owner_and_pets()
    _, restored_pets = import_data(export_data(owner, pets))
...
def test_roundtrip_preserves_multiple_pets():
    ...
    _, restored = import_data(export_data(owner, [pet1, pet2]))
...
def test_completed_field_roundtrips():
    ...
    _, restored = import_data(export_data(owner, [pet]))
